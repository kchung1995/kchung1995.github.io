...
덕분에 어떨 때 sealed를 쓰는 것이 확실히 편리하고 좋은지, 경험한 내용을 정리해 공유하고자 한다. `sealed` 한정자에 대한 전반적인 내용을 다루지만, 소개하는 예시는 `sealed interface`에 집중되어 있다. ~~sealed classs는 공유할 정도로 유용하게 활용한 적이 없다. 안 해본 걸 소개할 순 없으니... ㅋㅋ~~
...
// 강의 하위 도메인
interface Major : Lecture

interface General : Lecture

interface Graduate : Lecture
...
각 하위 도메인 별구현체도 살펴 보자. 내용이 너무 길어져서, 인터페이스의 필드를 오버라이드 (override) 하는 내용들은 주석으로 생략했다.
...
* `NoteController` 빈이 생성될 때, `NoteDetail`의 모든 구현체가 인스턴스화 된다. `List<NoteDetail>` 형태로 `noteDetails` 내에 담기낟.
* API가 호출되면, `noteDetails`에서 parameterd의 값에 대응하는 `NoteDetail`을 찾는다.
