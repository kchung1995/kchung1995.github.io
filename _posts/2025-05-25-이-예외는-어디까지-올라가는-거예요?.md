---
categories: posts
data: 2025-05-25 17:00
tags: [코틀린, 코루틴]
image: /assets/images/posts/2025-05-25-이-예외는-어디까지-올라가는-거예요/exception-propagation.png
---
...
* 병렬 (동시적) 처리
  * 위 코드는 실행 시 `item.forEach`의 내용을 suspend 없이 실행하며, 따라서 처리 속도가 `forEach`의 크기에 비례하게 된다.
...
![test01](/assets/images/posts/2025-05-25-이-예외는-어디까지-올라가는-거예요/test01.png)

## 시도와 해결 과정

### 1. coroutineScope 사용은 적합하지 않다. Coroutine Builder인 launch를 사용해 보자.

우선, 기존 코드의 `coroutineScope`를 `launch`로 변경하였다.

하술하겠지만, `coroutineScope`는 scope builder이다. 여기서 필요한 것은 list 내 각 num 별로 새 코루틴을 만들어서 각각 실행시키는 것이다.
...
![test02](/assets/images/posts/2025-05-25-이-예외는-어디까지-올라가는-거예요/test02.png)

동시성은 해결되었지만, 이번에는 예외 핸들링이 되지 않는다. 던진 예외를 잡지 못하는 모습이다.

![bugfix](/assets/images/posts/2025-05-25-이-예외는-어디까지-올라가는-거예요/bugfix.gif)
...
![test03](/assets/images/posts/2025-05-25-이-예외는-어디까지-올라가는-거예요/test03.png)
...
![exception-propagation](/assets/images/posts/2025-05-25-이-예외는-어디까지-올라가는-거예요/exception-propagation.png)

그렇다면, coroutine scope를 별도로 생성하지 않고, 이를 try-catch로 감싸 주면 어떨까?

이걸 위해서 scopeBuilder를 사용한다. scopeBuilder에는 앞에서 잠시 언급한  `coroutineScope` 외에도 `globalScope` 등이 존재하는데, 여기서는 단일 라이프사이클을 가지는 `coroutineScope`가 적합하다.
...
이렇게 해 주면, `callMethod()` 호출 시 예외가 발생한다면, `coroutineScope`까지만 예외가 전파될 것이다.

이를 try-catch로 감싸 주었으니, 의도한 바와 같이 동작할 것이다.

![test04](/assets/images/posts/2025-05-25-이-예외는-어디까지-올라가는-거예요/test04.png)
...
그런데 위 코드의 경우에는 `Thread.sleep()`이 **주석 처리**되어 있다. 그럼에도 불구하고 결과를 확인할 수 있었다.

이 말은 비동기 처리가 정상적으로 이루어지지 않았다는 것을 의미한다.

### 4. 가장 바깥을 CoroutineScope().launch로 감싸 주자

앞서 비동기 처리를 위해서는 `CoroutineScope`를 사용해서 (`coroutineScope` 아님) 새 코루틴을 만들었다 (`launch`). 새 코루틴을 만듦으로써 해당 코루틴을 별도 스레드에서 실행되도록 분리했었다.

이쯤에서 `CoroutineScope`와  `coroutineScope`의 차이점을 간단하게 짚어 보자. `CoroutineScope`는 코루틴이 소속되는 곳이며, 코루틴의 라이프사이클을 정의하기 위해 사용된다. `coroutineScope`는 suspend function 내에서 호출되며, 자식 코루틴들의 구조화된 동시성을 구성하기 위해 사용된다. `CoroutineScope`에 비해 라이프사이클이 더 짧기도 하다.

이 내용을 다시 적용해 보자. `CoroutineScope().launch { }`로 감싸 주면, 내부의 로직은 별도 코루틴으로 동시적으로 (concurrently) 동작하게 될 것이다.

![structured-concurrency](/assets/images/posts/2025-05-25-이-예외는-어디까지-올라가는-거예요/structured-concurrency.png)
...
![test05_01](/assets/images/posts/2025-05-25-이-예외는-어디까지-올라가는-거예요/test05_01.png)

비동기로 잘 처리된 것으로 보인다.

그렇다면 `Thread.sleep()`의 주석을 풀고 한번 더 확인해 보자.

![test05_02](/assets/images/posts/2025-05-25-이-예외는-어디까지-올라가는-거예요/test05_02.png)
...
위에서 설명한 바에 따르면, `calledMethod(num)` 호출 시 예외가 발생하면, `runTest`에까지 해당 예외가 전파됩니다. 그렇다면 `runTest`를 try-catch로 감싸 주었으니, 예외 처리가 정상 동작해야 한다.

![test_extra](/assets/images/posts/2025-05-25-이-예외는-어디까지-올라가는-거예요/test_extra.png)

예상대로 catch가 잘 된 것을 확인할 수 있다. 물론 기존에 정의했던 문제를 해결하는 적절한 방법은 아니다.

[This post is also avilable in English.](https://medium.com/@kchung1995/kotlin-coroutines-this-exception-escalates-up-to-33eefe708302)
